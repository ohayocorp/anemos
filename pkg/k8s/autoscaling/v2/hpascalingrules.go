...
type HPAScalingRules struct {
	...
	Tolerance *any `json:"tolerance,omitempty" yaml:"tolerance,omitempty"`
}

func NewHPAScalingRules() *HPAScalingRules {
	return &HPAScalingRules{}
}

func NewHPAScalingRulesWithSpec(spec *HPAScalingRules) *HPAScalingRules {
	
	return spec
}

func RegisterHPAScalingRules(jsRuntime *js.JsRuntime) {
	jsRuntime.Type(reflect.TypeFor[HPAScalingRules]()).JsNamespace("k8s.autoscaling.v2").Fields(
		js.Field("Policies"),
		js.Field("SelectPolicy"),
		js.Field("StabilizationWindowSeconds"),
		js.Field("Tolerance"),
	).Constructors(
		js.Constructor(reflect.ValueOf(NewHPAScalingRules)),
		js.Constructor(reflect.ValueOf(NewHPAScalingRulesWithSpec)),
	)
}